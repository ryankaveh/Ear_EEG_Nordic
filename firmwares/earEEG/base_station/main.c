...
#define MAX_USB_PACKET_LEN      251

#define NUS_SERVICE_UUID_TYPE   BLE_UUID_TYPE_VENDOR_BEGIN              /**< UUID type for the Nordic UART Service (vendor specific). */

static char ee_rx_buff[MAX_USB_PACKET_LEN];
static bool echo = 0;
static bool newline = 0;
static uint8_t mru_index = 0;
static char char_buffer;
static const char delimiter[2] = " ";
static char m_tx_buffer[NRF_DRV_USBD_EPSIZE*512];
 ...
 *
 * @details This function takes a list of characters of length data_len and prints the characters out on USB.
 * 
 */
static void ble_nus_chars_received_usb_print(uint8_t * p_data, uint16_t data_len)
{
    ret_code_t ret_val;
    ...
    for (int i = 0 ; i != 65 ; i++) 
    {
      sprintf(buf + (3*i), "%02x ", m_nus_data_array[i]);
    }
    buf[195] = '\0';
    */
    ...
    //ret_code_t ret = app_usbd_cdc_acm_write(&m_app_cdc_acm, buf, size_of_buff);
    if(ret != NRF_SUCCESS)
    {
...
static void ble_nus_c_evt_handler(ble_nus_c_t * p_ble_nus_c, ble_nus_c_evt_t const * p_ble_nus_evt)
{
    ...
    switch (p_ble_nus_evt->evt_type)
    {
        ...
        case BLE_NUS_C_EVT_NUS_TX_EVT:
            ble_nus_chars_received_usb_print(p_ble_nus_evt->p_data, p_ble_nus_evt->data_len);
            break;
...
// This isn't a service on its own. It's just some code making sure that we're only connecting to one other device running the same service.
                                    ...
                                    app_usbd_cdc_acm_user_event_t event)
{
    ...
    switch (event)
    {
        ...
        case APP_USBD_CDC_ACM_USER_EVT_RX_DONE:
        {
            ...
            echo = 1;
            mru_index = index-1;
            char_buffer = m_cdc_data_array[index-1];
            do
            {
                    ...
                    (index >= (m_ble_nus_max_data_len)))
                {
                    if (index > 1)
                    {
                        ...
                        do
                        {
                            ...
                            ret = ble_nus_c_string_send(&m_ble_nus_c, m_cdc_data_array, index);
                            strcpy(ee_rx_buff,m_cdc_data_array);  //store received data in my own buffer
                            newline = 1;
...
int main(void)
{
    ...
    for (;;)
    {
        ...
        if(echo)
        {
          size_t size = sprintf(m_tx_buffer, "%c",char_buffer);
          app_usbd_cdc_acm_write(&m_app_cdc_acm, m_tx_buffer, size);
          echo = 0;
        }
        idle_state_handle();
