...
-------> toggle LEDs
-------> Parse incoming R/W commands and construct SPI trx array with appropriate data
-------> Can trigger a timer to stream data to host device
-------> Toggle BLE into full speed (2M PHY) 

TO IMPLEMENT:
- SPI functionality
- SPI packetization

--> added spi event handler
--> need to connect ee_process_cmds to spi functionality
--> try out with ble only

TESTED:
- BLE transactions (back and forth between peripheral and central devices)
- BLE transactions at full speed

TO TEST:
- SPI transactions
- SPI and BLE transactions at full speed
 */
...
//SPI_p APP_SPIS_CS_PIN 31
//SPI_p APP_SPIS_MOSI_PIN 29
//SPI_p APP_SPIS_MISO_PIN 30
//SPI_p APP_SPIS_SCK_PIN 26
...
static uint32_t       timer_ticks = 15; // 8 ticks from a 32kHz clock should give me a transaction every ~250us
                       ...
                       void *                  p_context)
{
    ...
    if (spi_cmd_flag == SPI_READ_REG) {
      ...
      {
        NRF_LOG_INFO("inside spi handler - shouldn't be here");
...
static void trx_timer_handler(void * p_context)
{
  ...
  {
    base_idx = 0;
    ble_tx_buf[0] = ble_tx_buf[130] + 1;

    ble_tx_buf[4] = ble_tx_buf[134] + 2; // EEG0
    ble_tx_buf[6] = ble_tx_buf[6] + 4; // I0
    ble_tx_buf[8] = ble_tx_buf[8] + 3; // Q0
    ble_tx_buf[12] = ble_tx_buf[12] + 3; // EEG1
    ble_tx_buf[11] = ble_tx_buf[11] + 1; // EEG1
    ble_tx_buf[14] = ble_tx_buf[14] + 7; // I1
    ble_tx_buf[16] = ble_tx_buf[16] + 7; // Q1
    ble_tx_buf[20] = ble_tx_buf[20] + 3; // EEG2
    ble_tx_buf[22] = ble_tx_buf[22] + 7; // I2
    ble_tx_buf[24] = ble_tx_buf[24] + 7; // Q2
    ble_tx_buf[28] = ble_tx_buf[28] + 3; // EEG3
    ble_tx_buf[30] = ble_tx_buf[30] + 4; // I3
    ble_tx_buf[32] = ble_tx_buf[32] + 4; // Q3
    ble_tx_buf[36] = ble_tx_buf[36] + 1; // EEG4
    ble_tx_buf[38] = ble_tx_buf[38] + 1; // I4
    ble_tx_buf[40] = ble_tx_buf[40] + 1; // Q4
    ble_tx_buf[44] = ble_tx_buf[44] + 1; // EEG5
    ble_tx_buf[46] = ble_tx_buf[46] + 1; // I5
    ble_tx_buf[48] = ble_tx_buf[48] + 1; // Q5
    ble_tx_buf[52] = ble_tx_buf[52] + 1; // EEG6
    ble_tx_buf[54] = ble_tx_buf[54] + 1; // I6
    ble_tx_buf[56] = ble_tx_buf[56] + 1; // Q6
    ble_tx_buf[60] = ble_tx_buf[60] + 1; // EEG7
    ble_tx_buf[62] = ble_tx_buf[62] + 1; // I7
    ble_tx_buf[64] = ble_tx_buf[64] + 1; // Q7

  } else if (spi_trx_counter == 1) {
    ble_tx_buf[65] = ble_tx_buf[0] + 1;

    base_idx = 65;
    ble_tx_buf[base_idx + 4] = ble_tx_buf[4] + 2; // EEG0
    ble_tx_buf[base_idx + 6] = ble_tx_buf[6] + 4; // I0
    ble_tx_buf[base_idx + 8] = ble_tx_buf[8] + 3; // Q0
    ble_tx_buf[base_idx + 12] = ble_tx_buf[12] + 3; // EEG1
    ble_tx_buf[base_idx + 11] = ble_tx_buf[11] + 1; // EEG1
    ble_tx_buf[base_idx + 14] = ble_tx_buf[14] + 7; // I1
    ble_tx_buf[base_idx + 16] = ble_tx_buf[16] + 7; // Q1
    ble_tx_buf[base_idx + 20] = ble_tx_buf[20] + 3; // EEG2
    ble_tx_buf[base_idx + 22] = ble_tx_buf[22] + 7; // I2
    ble_tx_buf[base_idx + 24] = ble_tx_buf[24] + 7; // Q2
    ble_tx_buf[base_idx + 28] = ble_tx_buf[28] + 3; // EEG3
    ble_tx_buf[base_idx + 30] = ble_tx_buf[30] + 4; // I3
    ble_tx_buf[base_idx + 32] = ble_tx_buf[32] + 4; // Q3
    ble_tx_buf[base_idx + 36] = ble_tx_buf[36] + 1; // EEG4
    ble_tx_buf[base_idx + 38] = ble_tx_buf[38] + 1; // I4
    ble_tx_buf[base_idx + 40] = ble_tx_buf[40] + 1; // Q4
    ble_tx_buf[base_idx + 44] = ble_tx_buf[44] + 1; // EEG5
    ble_tx_buf[base_idx + 46] = ble_tx_buf[46] + 1; // I5
    ble_tx_buf[base_idx + 48] = ble_tx_buf[48] + 1; // Q5
    ble_tx_buf[base_idx + 52] = ble_tx_buf[52] + 1; // EEG6
    ble_tx_buf[base_idx + 54] = ble_tx_buf[54] + 1; // I6
    ble_tx_buf[base_idx + 56] = ble_tx_buf[56] + 1; // Q6
    ble_tx_buf[base_idx + 60] = ble_tx_buf[60] + 1; // EEG7
    ble_tx_buf[base_idx + 62] = ble_tx_buf[62] + 1; // I7
    ble_tx_buf[base_idx + 64] = ble_tx_buf[64] + 1; // Q7

  } else if (spi_trx_counter == 2) {

    ble_tx_buf[130] = ble_tx_buf[65] + 1;

    base_idx = 130;
    ble_tx_buf[base_idx + 4] = ble_tx_buf[65 + 4] + 2; // EEG0
    ble_tx_buf[base_idx + 6] = ble_tx_buf[65 + 6] + 4; // I0
    ble_tx_buf[base_idx + 8] = ble_tx_buf[65 + 8] + 3; // Q0
    ble_tx_buf[base_idx + 12] = ble_tx_buf[65 + 12] + 3; // EEG1
    ble_tx_buf[base_idx + 11] = ble_tx_buf[65 + 11] + 1; // EEG1
    ble_tx_buf[base_idx + 14] = ble_tx_buf[65 + 14] + 7; // I1
    ble_tx_buf[base_idx + 16] = ble_tx_buf[65 + 16] + 7; // Q1
    ble_tx_buf[base_idx + 20] = ble_tx_buf[65 + 20] + 3; // EEG2
    ble_tx_buf[base_idx + 22] = ble_tx_buf[65 + 22] + 7; // I2
    ble_tx_buf[base_idx + 24] = ble_tx_buf[65 + 24] + 7; // Q2
    ble_tx_buf[base_idx + 28] = ble_tx_buf[65 + 28] + 3; // EEG3
    ble_tx_buf[base_idx + 30] = ble_tx_buf[65 + 30] + 4; // I3
    ble_tx_buf[base_idx + 32] = ble_tx_buf[65 + 32] + 4; // Q3
    ble_tx_buf[base_idx + 36] = ble_tx_buf[65 + 36] + 1; // EEG4
    ble_tx_buf[base_idx + 38] = ble_tx_buf[65 + 38] + 1; // I4
    ble_tx_buf[base_idx + 40] = ble_tx_buf[65 + 40] + 1; // Q4
    ble_tx_buf[base_idx + 44] = ble_tx_buf[65 + 44] + 1; // EEG5
    ble_tx_buf[base_idx + 46] = ble_tx_buf[65 + 46] + 1; // I5
    ble_tx_buf[base_idx + 48] = ble_tx_buf[65 + 48] + 1; // Q5
    ble_tx_buf[base_idx + 52] = ble_tx_buf[65 + 52] + 1; // EEG6
    ble_tx_buf[base_idx + 54] = ble_tx_buf[65 + 54] + 1; // I6
    ble_tx_buf[base_idx + 56] = ble_tx_buf[65 + 56] + 1; // Q6
    ble_tx_buf[base_idx + 60] = ble_tx_buf[65 + 60] + 1; // EEG7
    ble_tx_buf[base_idx + 62] = ble_tx_buf[65 + 62] + 1; // I7
    ble_tx_buf[base_idx + 64] = ble_tx_buf[65 + 64] + 1; // Q7
  }
